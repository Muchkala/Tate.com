                ...
                return  <div className='basket' key={id}>
                            <Grid2 container spacing={0} sx={{width:'100%', color:'black', border:'1px solid gray'}}>
                                ...
                                <div style={{width:'100%', display:'flex' ,gap:'10px'}} className="QTY">
                                    ...
                                    <Button className='QTYbutton' variant='outlined' onClick={() => dispatch({type: 'MINUS_QTY', payload: id})}>-</Button>
                                    <span>{QTY}</span>
                                    <Button  className='QTYbutton'  variant='outlined' onClick={() => dispatch({type: 'PLUS_QTY', payload: id})}>+</Button>
                                </div>
                                <h4>price : {price * QTY} <span style={{fontFamily:'roboto'}}>$</span></h4>
                                <Button className='BSCbutton' variant='outlined' style={{alignSelf:'end',marginRight:'10px'}} onClick={() => dispatch({type: 'REMOVE_BASKET', payload: id})}><DeleteOutline/></Button>
                                </Grid2>
                            </Grid2>
                        </div>
